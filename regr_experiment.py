...
def config():
    ...
    NUM_HIDDEN = 800     # Number of hidden units in the hidden layer
...
@ex.capture
def regr_model(y1,y2, keep_prob, mu, NUM_REPS, NUM_HIDDEN, NUM_OUTPUTS, REGULARIZE_BIAS):
    ...
    W_fc1 = weight_variable([1200, NUM_HIDDEN])
    b_fc1 = bias_variable([NUM_HIDDEN])

    #h_pool3_flat = tf.reshape(h_pool3, [-1, 4*4*DEPTH_3])
    h_fc1 = tf.nn.relu(tf.matmul(tf.concat(1, [y1, y2]), W_fc1) + b_fc1)
    ...
    y_regr = tf.nn.softmax(tf.matmul(h_fc1_drop, W_fc2) + b_fc2)

    loss =  mu*(tf.nn.l2_loss(W_fc1) + tf.nn.l2_loss(W_fc2))

    if REGULARIZE_BIAS:
        loss += mu*(tf.nn.l2_loss(b_fc1) + tf.nn.l2_loss(b_fc2))
...
def construct_graph(PIC_WIDTH, PARAMS1, PARAMS2, NUM_INPUTS, NUM_OUTPUTS, LEARNING_RATE, NUM_REPS):
    ...
    with graph.as_default():
        ...
        pred, rg = regr_model(y1, y2, keep_prob)
...
def train(START_ITER, ITERS, RUN_NAME, MODEL_LOAD_PATH, DATA_AUGMENTATION,
    ...
    with tf.Session(graph=graph) as session:
        ...
        for i in range(START_ITER,START_ITER+ITERS):
            Xt,yt, _ = minibatch(train, train_labels, size=100, noise_std=TRAIN_LABEL_NOISE_STD)
